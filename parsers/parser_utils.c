...
struct parser_definition
parser_utils_strcmps(const char *s) {
    const size_t n = strlen(s);

    struct parser_state_transition **states   = calloc(n + 2, sizeof(*states));
    size_t *nstates                           = calloc(n + 2, sizeof(*nstates));
    struct parser_state_transition *transitions= calloc(2 *(n + 2),
                                                        sizeof(*transitions));
    if(states == NULL || nstates == NULL || transitions == NULL) {
        free(states);
        free(nstates);
    ...
    for(size_t i = 0; i < n; i++) {
        ...
        transitions[i * 2 + 0].when = s[i];
        transitions[i * 2 + 0].dest = dest;
        transitions[i * 2 + 0].act1 = i + 1 == n ? eq : may_eq;
        transitions[i * 2 + 2].when = ANY;
        transitions[i * 2 + 2].dest = st_neq;
        transitions[i * 2 + 2].act1 = neq;
        states     [i]              = transitions + (i * 2 + 0);
        nstates    [i]              = 2;
    }
    // EQ
    transitions[(n + 0) * 2].when   = ANY;
    transitions[(n + 0) * 2].dest   = st_neq;
    transitions[(n + 0) * 2].act1   = neq;
    states     [(n + 0)]            = transitions + ((n + 0) * 2 + 0);
    nstates    [(n + 0)]            = 1;
    // NEQ
    transitions[(n + 1) * 2].when   = ANY;
    transitions[(n + 1) * 2].dest   = st_neq;
    transitions[(n + 1) * 2].act1   = neq;
    states     [(n + 1)]            = transitions + ((n + 1) * 2 + 0);
    nstates    [(n + 1)]            = 1;
...
void
parser_utils_strcmps_destroy(const struct parser_definition *p) {
    free((void *)p->states[0]);
    free((void *)p->states);
    free((void *)p->states_n);
}
